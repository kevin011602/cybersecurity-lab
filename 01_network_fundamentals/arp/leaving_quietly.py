...
def leaving_quietly():
    ...
    arp_response.hwdst = "00:50:56:f7:1e:e8"  # Router's MAC address
    
    # Set the real MAC address of the victim (so the router learns the correct binding)
    arp_response.hwsrc = "00:0c:29:9c:0e:34"  # Victim's MAC address
    
    # Set the victim's IP address as the source IP
    arp_response.psrc = "192.168.58.137"  # Victim's IP address
    ...
    arp_response.pdst = "192.168.58.148"  # Victim's IP address
    
    # Set the victim's MAC address as the destination hardware
    arp_response.hwdst = "00:0c:29:81:26:41"  # Victim's MAC address
    
    # Set the real MAC address of the router (to reestablish trust)
    arp_response.hwsrc = "00:50:56:f7:1e:e8"  # Router's MAC address
    
    # Set the router's IP address as the source IP
    arp_response.psrc = "192.168.58.2"  # Router's IP address
